...
VocaDox incorporates the following third-party software. This file is
derived from `compliance/dependency-inventory.yml` and
`compliance/container-inventory.yml` — those files are the machine-readable
source of truth; run `python compliance/check_licenses.py` to verify this
list against the current policy. Last generated: 2026-08-17.

No dependency or container image in this list is licensed under a
...
| python:3.11.10-slim-bookworm | PSF-2.0 (interpreter); mixed OS-package licenses (Debian base layer) |
| node:20.18.1-bookworm-slim | MIT (runtime); mixed OS-package licenses (Debian base layer) |
| nginx:1.27.3-alpine3.20 | BSD-2-Clause (nginx license) |
