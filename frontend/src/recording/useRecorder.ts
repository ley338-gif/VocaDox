 ...
 *
 * `source: "both"` records the local microphone (the person running the
 * session) together with the shared tab/screen's audio (e.g. the other
 * call participants) as a single take. `MediaRecorder` only honors one
 * audio track per browser implementation, so the two raw streams are
 * mixed down to one track via a Web Audio `AudioContext` +
 * `createMediaStreamDestination` before recording — see
 * `mixAudioStreams()`.
 */
...
export type AudioSource = "microphone" | "system-audio" | "both";
...
export function isCombinedAudioCaptureSupported(): boolean {
  return isSystemAudioCaptureSupported() && Boolean(getAudioContextCtor());
}
...
export function useRecorder() {
  ...
  // "both" mode: the raw mic/display-audio streams feeding the mix, and the
  // AudioContext doing the mixing — separate from streamRef (the mixed
  // destination stream actually handed to MediaRecorder) so both can be torn
  // down independently on stop/unmount.
  const mixInputStreamsRef = useRef<MediaStream[]>([]);
  ...
  const requestPermission = useCallback(
    async (source: AudioSource = "microphone") => {
      ...
      if (source === "both" && !isCombinedAudioCaptureSupported()) {
        dispatch({ type: "UNSUPPORTED" });
        return;
      }
      dispatch({ type: "REQUEST_PERMISSION" });
      try {
        // Tracks whose "ended" event should be treated as a device
        // disconnect. For "both" this is the raw mic/display tracks, not
        // the mixed destination stream's track (which is generated by the
        // audio graph and never fires "ended" on its own).
        let deviceTracks: MediaStreamTrack[];
        let rawStream: MediaStream;
        if (source === "system-audio" || source === "both") {
          ...
          const displayAudio = new MediaStream(displayStream.getAudioTracks());
          ...
          if (source === "both") {
            ...
            mixContext.createMediaStreamSource(displayAudio).connect(destination);
            mixContextRef.current = mixContext;
            mixInputStreamsRef.current = [micStream, displayAudio];
            deviceTracks = [...micStream.getAudioTracks(), ...displayAudio.getAudioTracks()];
            rawStream = destination.stream;
          } else {
            deviceTracks = displayAudio.getAudioTracks();
            rawStream = displayAudio;
          }
        } else {
          rawStream = await navigator.mediaDevices.getUserMedia({ audio: true });
          deviceTracks = rawStream.getAudioTracks();
        }
  ...
  const stopInputStreams = useCallback(() => {
    streamRef.current?.getTracks().forEach((track) => track.stop());
    mixInputStreamsRef.current.forEach((stream) => stream.getTracks().forEach((track) => track.stop()));
    mixInputStreamsRef.current = [];
    mixContextRef.current?.close().catch(() => undefined);
  ...
  const stop = useCallback(() => {
    ...
    dispatch({ type: "STOP" });
  }, [dispatch, stopInputStreams, stopLevelLoop]);
  ...
  }, [stopInputStreams, stopLevelLoop]);
